...
const statusConfig: Record<string, { label: string; color: string }> = {
  TODO: { label: "To Do", color: "border-t-zinc-400" },
...
export function KanbanColumn({ status, tasks, onDrop, onTaskClick }: KanbanColumnProps) {
  const config = statusConfig[status] || { label: status, color: "border-t-zinc-400" };
  ...
  return (
    <div
      className={`flex w-72 flex-shrink-0 flex-col rounded-xl border-t-2 bg-zinc-50 dark:bg-zinc-800/50 ${config.color}`}
    ...
    >
      ...
      <div className="flex flex-col gap-2 overflow-y-auto px-3 pb-3">
        ...
        {tasks.length === 0 && (
          <div className="rounded-lg border border-dashed border-zinc-300 p-6 text-center text-xs text-zinc-400 dark:border-zinc-600">
            Drop tasks here
          </div>
        )}
      </div>
